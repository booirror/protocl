...
public class JavaStruct {
	...
	private String basicOperateMethod() {
		...
		for (MsgField field : classInfo.getMsgFields()) {
			...
			if (field.getDefType() == FieldType.VECTOR) {
				methodBuf.append(nextLine(2, 0));
	...
	private String fromByteBufferMethod() {
		...
		for (MsgField field : classInfo.getMsgFields()) {
			...
			methodBuffer.append(String.format("this.%s = ", field.getDefName()));
			if (field.getDefType() == FieldType.VECTOR) {
				methodBuffer.append(String.format("readArray(buf, %s.class", javaField.getGenericType()));
	        } else if (field.getDefType() == FieldType.MESSAGE) {
	        	methodBuffer.append(String.format("read%s(buf);", firstCharUpCase(field.getDefName())));
	        } else {
	...
	private String toByteBufferMethod() {
		...
		for (MsgField field : classInfo.getMsgFields()) {
			...
			if (field.getDefType() == FieldType.VECTOR) {
				methodBuffer.append(String.format("writeArray(builder, %s);", field.getDefName()));
			} else if (field.getDefType() == FieldType.MESSAGE) {
				methodBuffer.append(String.format("writeMessage(builder, %s);", field.getDefName()));
			} else {
				methodBuffer.append(String.format("write%s(builder, , %s);", firstCharUpCase(field.getDefType().def()), field.getDefName()));
			}
		}
	...
	private String toStringMethod() {
		...
		for (MsgField field : classInfo.getMsgFields()) {
			...
			if (field.getDefType() == FieldType.VECTOR) {
				strBuf.append(nextLine(1, 2));
				strBuf.append(String.format("if (%s  != null) {", field.getDefName()));
				
				strBuf.append(nextLine(1, 3));
				strBuf.append("strBuf.append(\"\\n").append(field.getDefName()).append(":\");");

				strBuf.append(nextLine(1, 3));
				strBuf.append(String.format("for (%s data : %s) {", javaField.getGenericType(), field.getDefName()));
				...
				strBuf.append(nextLine(1, 2));
				strBuf.append("strBuf.append(\"\\n\\t");
				strBuf.append(field.getDefName());
